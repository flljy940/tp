...
2. Deleting a person while the list is filtered with persons shown

   1. Prerequisites: Filter persons using a filter condition of your choice (eg. `filter-payment unpaid` command to filter persons with `Not Paid` payment status).

   2. Test case: `delete 1`<br>
      Expected: First contact is deleted from the list of filtered persons. Details of the deleted contact shown in the status message. Timestamp in the status bar is updated.
   ...
   5. `list` after deleting person(s) from the filtered list should show the full list of persons in the original unfiltered list excluding the deleted person(s).
